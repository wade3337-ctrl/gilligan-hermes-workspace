...
- 📝 **Aspen — retention/marketing/BD agent** (repurposed Arduino Herman; PLANNING 2026-07-15) — teardown+rebuild of the Arduino board into **Aspen**: customer **retention → marketing → BD** (in sequence, retention first). Keep board+Hermes+GPT, wipe memories, fresh `aspen-knowledge` wiki, Telegram (Skipper+Nate→team), TRIM IT read-only, self-learning loop, crew-built. Metric = churn/at-risk + repeat-rate. ▶️ **[[aspen-retention-agent]]** 🔗 [[brent-agent]], [[herman-agent]], [[arbor-core-crew-infra]] ⚠️ **plan awaiting Skipper sign-off — nothing built.**
