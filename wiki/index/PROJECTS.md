...
- 📦 **MuniBot data warehouse** — Brent's municipal bid data → MuniBot. Receiver built+proven (bind-mounted write-only rsync warehouse). ▶️ [[munibot-data-warehouse]] 🟢 **TRANSFER RUNNING (2026-07-17):** the ~42GB was already staged on Skipper's home laptop (`%LOCALAPPDATA%\MuniSync\data`) → `SyncMuni-LOCAL` (v5.1, local-source) rsyncing it straight to `/opt/data/municipal-archive/` over Tailscale; ~30GB/14.5k files landed, climbing; watchdog emails on done/stall. MuniBot SOUL+memory repointed to the warehouse path.
- 🔵 **MuniBot Smart Municipal Bidding Tool** (scoping 2026-07-17) — AI-in-the-loop tool: drop an RFP packet (e.g. Long Beach PW25-648) → MuniBot auto-fills a priced bid draft for team review. Two layers (Pricing Brain + Bid Filler); win-first, TPH margin floor (runtime, multi-year Irvine-trap aware); weights in [[pricing-guide-bid-prefill]] (Price Buddy) + comps + warehouse bid history. **Boss Hermes builds it (crew+loop).** ▶️ **[[munibot-smart-bidding-tool]]** ✅ **BUILT (Boss Hermes) + INSTALLED on MuniBot 2026-07-17** — 3 scripts + SKILL.md in MuniBot's container (`/opt/data`), venv + pymupdf/openpyxl; v1→v2→SKILL palm→**v3 Price Buddy (CycleTimeEach)**, all smoke-tested. 🧪 **REAL TEST staged** — Brent's 8 city bids sorted (BEFORE-bids vs WINNING-answers, `inbox-pull/city-bids-2025-2026/`); ⏳ asked Herman to add warehouse-inventory lookup, then run the 8-city test & score vs GSTS's actual bids. See [[munibot-smart-bidding-tool]].
