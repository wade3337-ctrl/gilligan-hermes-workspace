...
- [[path-to-25m-2026]] — 📈 run-rate + capacity math to the 2026 goal. **H1 $11.22M = 91.6% of plan; the shortfall is mostly VOLUME but efficiency is NOT quite on target — on clocked payroll hours H1 is $122.01 vs the $130 target (Q1 $118.76 → Q2 $125.10). $2.2M/mo lands $24.42M; required is $2,346,424/mo against the authoritative $25,300,976 goal.** ERP reconciles to Dimitry's books within 1.28%. ⛔ The old *"TPH $130.19 → efficiency is ON target"* line is retired — that was the scheduled-hour basis and overstated labor efficiency by ~11%.
- [[bod-commitment-dashboard]] — 🖥️ the Skipper's cockpit for the four commitments he put in front of the Board. **BUILT on play 2026-07-29**, gated to UserID 9/376. Tile 1 tracks **15 NET ADDS off a frozen 79 baseline** (not the absolute 91 — that came from a holiday-contaminated mean); CFO revenue is stored, not derived → [[monthly-cfo-reconciliation]].
- [[crewsheet-acthours-is-the-estimate]] — ⛔ `CrewSheets.ActHours` is the ESTIMATE (= `EstHours` on 98–99% of sheets); never call it actual labor.
- [[timekeeping-live-nov-2025]] — clocked hours only exist from Nov 2025 → no year-over-year on them before Nov 2026.
- [[crew-assignment-drift]] — stale crew-member assignment poisons per-crew hours/TPH (dead crew records, one literally named `DELETE`) → company level only.
- [[pending-crewsheet-closeout-gap]] — the clocked-vs-sheeted hours gap is **internal non-productive time** (Safety/Meetings, Yard, OJT booked to a crew that never completes), not unclosed paperwork.
...
- [[cfo-financials-derivation]] — 🧮 **what we can DERIVE from the CFO's own file vs what we must ask for** (`business-plan/derive-financials.py`, controls-gated). Reclass rebuilt to **$0.00**; **deal AGP = GP before D&A = 49.92%** vs FTI's ~50%. **At current pace the earnout is $0 — floor is $335K away**; the goal vs his budget figure is worth **$1.45M**. Only Base EBITDA + the QoE adjustments still need asking.
