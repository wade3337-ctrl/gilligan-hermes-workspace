...
- **On-demand dashboard capture (WIRED, awaiting Brent live test):** Brent asks MuniBot to see a board → it logs into TRIM IT (play), screenshots the live dashboard, auto-dismisses the welcome modal, and sends the PNG on Telegram via `MEDIA:<path>`.
  ...
  - Build: `~/.munibot/home/dashboard-capture/{capture.js,capture.sh}` (Playwright/chromium; board→URL map; `city-budgets` → `/GSTS/Dashboard-CityBudgets.cfm`) + skill `productivity/dashboard-capture`. Extensible to more boards by adding to the map. Detail → [[segment-margin-analysis]] is unrelated; capture technique in [[PLAYBOOK]] infra/agents/dashboard-capture.
