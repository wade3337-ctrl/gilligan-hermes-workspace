...
## 2026-07-21 — vision fixed + on-demand dashboard capture (Skipper build)
- **Vision (reading images) FIXED:** MuniBot told Brent it couldn't read screenshots — root cause was `auxiliary.vision` routed to the main model glm-5.2 (Z.ai 403 on image input) → all `vision_analyze`/`browser_vision` failed. Repointed to **Gemini 3.1 Pro** (`auxiliary.vision.provider: gemini`, `model: gemini-3.1-pro-preview`) + `GEMINI_API_KEY` (PAID key) in `.env`. Restart picks up .env from the /opt/data mount. Verified + Skipper live-tested. See [[PLAYBOOK]] infra/agents/hermes-vision.
- **On-demand dashboard capture (WIRED, awaiting Brent live test):** Brent asks MuniBot to see a board → it logs into TRIM IT (play), screenshots the live dashboard, auto-dismisses the welcome modal, and sends the PNG on Telegram via `MEDIA:<path>`.
  - **Login persistence fix:** play refresh had WIPED the custom `munibot@`/`hermes 90377` `flow.Users` logins. Solution (Skipper): point bots at an EXISTING prod account that survives refresh. MuniBot now uses shared test acct **`GSTCTest` (UserID 205)**; Gilligan uses **`TrimIT` (UserID 1)** (already on the dashboard allowlist). Creds: `~/.munibot/home/.secrets/trimit-web-login.env`. Passwords plaintext in `flow.Users` ([[herman-trimit-login]]).
  - Granted GSTCTest dashboard access = one INSERT into `Workbench.dbo.DashboardAccess` (allowlist by UserID; needs admin sqlcmd).
  - Build: `~/.munibot/home/dashboard-capture/` — `capture.sh <free-text board name>` + `refresh-boards.sh`. **ANY board by name, hands-off:** `refresh-boards.js` logs in once and auto-builds `boards.json` (name→URL from each dashboard's own `<title>` + filename alias) for all ~12 TRIM IT dashboards; `capture.js` fuzzy-matches Brent's words → returns `OK <png>` (→ reply `MEDIA:<png>`), `AMBIG: a|b|c` (ask which), or `NOMATCH: <list>`. Welcome modal (`#welcomeOverlay`/`#wcGotIt`) auto-dismissed on every board. No hand-maintained URL map — re-run refresh only if boards are added. Skill `productivity/dashboard-capture` v2. Capture technique in [[PLAYBOOK]] infra/agents/dashboard-capture.
