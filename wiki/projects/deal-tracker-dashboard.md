...
**✅ ACCRUAL RESOLVED (2026-07-22):** wired the LIVE accrual bridge from `dbo.GetPeriodAccrual` (the corrected, billing-cycle-aware muni accrual Steve confirmed + shipped to prod — see [[steve-recon-b-municipal-accrual]]). Adjusted Actual YTD = invoiced $12.01M + current-period accrual $58K − Dec-2025 baseline $161K = **$11.91M**. No Brent feed needed. Current period is partial-month (accrual grows toward month-end). Uncovered gap → **$4.73M**.
**▶️ STILL OPEN:** (1) live calendar-muni calc to replace the $3.74M config (engine-FY recon ~$3.19M) · (2) confirm sold-status list (46,109) w/ Herman (firm-WO recompute $1.5M under his snapshot) · (3) per-month coverage cols + drill-throughs (Herman §11 full monthly table incl. per-month accrual cols) not yet built.
