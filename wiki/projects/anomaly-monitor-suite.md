...
**Context:** emails dark since 7/25 (below) = Skipper "flying blind." Tonight proved prod is an open read source ([[prod-web-read-access]]) and split the fix into two tracks.
- 🧭 **The correction that mattered more than the build: the emails were HELD, not BROKEN.** All three team crons had been *commented out* on 7/25 for stale data — 9 days dark. The machinery (endpoint auth, figures, send path) worked the whole time. I had started designing a **scrape he does not need**, and owed him that correction out loud: the fix is un-holding onto data worth sending, not building a second path. → LESSONS 2026-08-03.
