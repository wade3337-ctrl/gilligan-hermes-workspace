...
🧭 **PRE-BID VISIBILITY — CORE PITCH REFRAME (2026-08-08, Skipper-corrected, standing up before Mon 8/10 FPC meeting):** The build is NOT "TRIM IT is blind pre-bid, Bigin adds sight." **TRIM IT ALREADY HAS pre-bid visibility — the RFP queue + travelers carry `EstValue`, `ProposalDueDate`, `NextActionDate`, `BoardMeetingDate`, `TotalFollowups`.** The gap is ORGANIZATION, not sight. Live proof (2026-08-08, `RFPs` table): 1.69M rows, `DateComplete` unmaintained (1.686M "open"), only **174** with a future `ProposalDueDate` and those carry **NULL EstValue**. So the pre-bid data exists but is unusable as-is. **Bigin/Aspen's job = ORGANIZE existing TRIM IT visibility into a clean, rep-owned, dollar-weighted, forecastable pipeline** — this is what fills FPC's "Proj. Commercial = TBD" bridge line (the forward commercial they can't yet forecast). Pitch: "the pipeline data already lives in TRIM IT; we're structuring it," NOT "building sight from blindness." Source tables to pull pre-bid from: `RFPs` (+ RFPActions/RFPPackages) and Projects in InProcess/Pending (the Sales Cockpit base). → FPC prep `inbox-pull/fpc-meeting-2026-08-10/PREP-BRIEF.md` §3.

🏛️ **NEW BUILD REQUIREMENT (2026-08-08, from FPC prep) — FORWARD MUNICIPAL / NTE-vs-PO:** TRIM IT has NO NTE (contract-ceiling) field — it stores only the PO amount, and only once Brent has the PO in hand. So forward municipal revenue is invisible to the ERP by design; the sales-report forecast = NTE÷12, lives only in Nate/Brent's spreadsheets. **The Bigin/Aspen layer should own the NTE ceiling + PO-draw tracking + a smarter forward forecast** (not flat ÷12). Full spec: `aspen-stack/MUNICIPAL-NTE-vs-PO-build-note.md`. Ties to the Collections lane (both are the AR/municipal-money half of the pipeline).
