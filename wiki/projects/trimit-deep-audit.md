...
> 🧭 **Sibling effort:** [[trimit-menu-cleanup]] — pruning the TRIM IT left-nav (the ~119 profile menu links; 9 confirmed-dead links to remove first). Same prod-only-change constraint (`AppForms`/`MyAppForms` revert on the play refresh), same test-on-a-frozen-`GSTS_cleanup`-copy pattern.
