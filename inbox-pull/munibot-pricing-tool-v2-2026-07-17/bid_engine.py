...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
WITH BandData AS (
    SELECT wol.Price, wol.EstTPH, wol.TrimMinutes, wol.SizeCode,
        CASE
    ...
    WHERE wo.StatusDefID = 48 AND wol.StatusDefID = 68
    AND wol.Price > 0 AND lst.ServiceClassID = 1
...
SELECT Band, COUNT(*) AS N,
    ...
    CASE WHEN AVG(EstTPH) > 0 THEN ROUND(AVG(Price) / AVG(EstTPH), 3) ELSE NULL END AS EstHours,
    CASE WHEN AVG(EstTPH) > 0 THEN ROUND(AVG(Price) / AVG(EstTPH) * 130.0, 2) ELSE NULL END AS BlendedFloor,
    CASE WHEN AVG(EstTPH) > 0 THEN ROUND(AVG(Price) / AVG(EstTPH) * 130.0 * 0.75, 2) ELSE NULL END AS GridFloor
FROM BandData WHERE Band IS NOT NULL
GROUP BY Band
ORDER BY CASE Band WHEN '0-6' THEN 1 WHEN '7-12' THEN 2 WHEN '13-18' THEN 3 WHEN '19-24' THEN 4 WHEN '24-30' THEN 5 WHEN '31+' THEN 6 END;"""
    ...
    for r in rows:
        ...
        result[band] = {
            ...
            'est_tph': float(r.get('AvgEstTPH', 0)), 'trim_mins': float(r.get('AvgTrimMinutes', 0)),
            'est_hours': float(r.get('EstHours', 0)), 'blended_floor': float(r.get('BlendedFloor', 0)),
