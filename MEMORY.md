...
🎯 **DEPLOY (2026-07-19 final-verified): V1.5 7-dashboard package HANDOFF-READY** → `arbor-stack/release-candidates/DEPLOY-PACKAGE-V15-DASHBOARDS.md` (now incl. **Crew Performance #7** + Sales-Performance rename; full manifest/smoketest/security re-run clean as a 7-set; folded in the missing `CityBudgetsRenewals.data.cfm` include — no new DB step; **RGC still OUT — needs rework**). Gated ONLY on Skipper's green-light to Jordan/Travis.
